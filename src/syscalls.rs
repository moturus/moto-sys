...
impl SysCtl {
    ...
    pub const OP_SET_CPU_AFFINITY: u8 = 6;
    ...
    #[cfg(feature = "userspace")]
    pub fn set_cpu_affinity(thread: SysHandle, cpu: Option<u32>) -> Result<(), ErrorCode> {
        let result = do_syscall(
            pack_nr_ver(SYS_CTL, Self::OP_SET_CPU_AFFINITY, 0, 1),
            thread.as_u64(),
            match cpu {
                Some(cpu) => cpu as u64,
                None => u64::MAX,
